...
public class EightQueens implements Cloneable {
    ...
    public boolean setQueens(int queensRemaining){
        ...
        if (fitsInPlace) {
            ...
            setQueen(queenRow-1, queenColumn-1);
    ...
    public int updateThreatened(int row, int column, int cut){
        if (cut == 0){ //If cut == 0, the update is about adding threats rather than removing threats from the array
            ...
            //while loop goes down on a negative slope to add those compromised positions diagonally
    ...
    //returns true if queens are already in illegal position
    public boolean givenQueens() {
        for (int i = 0; i < this.chessBoard.length; i++){
            for (int j = 0; j < this.chessBoard[i].length; j++){
                if (chessBoard[i][j] == 'Q'){
                    if (!checkAvailability((8*(i)+j+1), new int[0])){
                        return true;
                    }
